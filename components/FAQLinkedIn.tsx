...
export function FAQLinkedIn() {
  const { faq } = linkedinContent;
          ...
          {faq.map((item, i) => {
            ...
            return (
              <article key={i} className="rounded-xl border border-slate-200 overflow-hidden">
                ...
                >
                  {item.q}
                  <svg className={`h-5 w-5 shrink-0 transition ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
